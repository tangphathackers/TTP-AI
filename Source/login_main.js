// login_main.js — Phiên bản gốc + Nhạc nền + Key động
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // === TẠO NỘI DUNG ĐỘNG CHO HINTS ===
  (function generateKeyHints() {
    const hintsContainer = document.getElementById('key-hints-container');
    if (!hintsContainer) return;

    // --- BẠN CÓ THỂ CHỈNH SỬA THÔNG TIN KEY TẠI ĐÂY ---
    const FREE_KEY_VALUE = "TTP";
    const VIP_KEY_URL = "https://zalo.me/g/ichtzt219";
    // ----------------------------------------------------

    const hintsHTML = `
      <span class="free-hint">🔑 Key miễn phí: <strong>${FREE_KEY_VALUE}</strong></span>
      <span class="vip-hint"> | VIP Key 👉 <a href="${VIP_KEY_URL}" target="_blank" rel="noopener noreferrer">Liên hệ Zalo</a></span>
    `;
